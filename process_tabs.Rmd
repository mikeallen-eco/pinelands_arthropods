...
# sum the reads across all negatives
sum(select(cn2, starts_with("sample"))) # 5208

# get the max read count of each seqid present in the negatives
colnames(cn2) # check which columns contain negative samples
cn2$seqmax <- apply(cn2[,10:26],1, max)

# write.csv(zs2, file = "data/pinebugs.samples.step1.onlyarth.C.csv", row.names = F)
# write.csv(zn2, file = "data/pinebugs.negatives.step1.onlyarth.C.csv", row.names = F)
...
colnames(zs3) # 10:45 are the samples
...
sum(select(zs3, starts_with("sample"))) # 3177355
rm(zs3a)
...
# write.csv(zs3, file = "output/fish2023.samples.step2.onlyfish.negsub.Z.csv", row.names = F)
...
colnames(cs3) # 10:45 are the samples
...
# count remaining reads in samples
sum(select(cs3, starts_with("sample"))) # 1720363
rm(cs3a)
...
# write.csv(cs3, file = "output/pinebugs.samples.step2.onlyarth.negsub.C.csv", row.names = F)
