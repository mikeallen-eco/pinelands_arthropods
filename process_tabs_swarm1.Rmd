...
# make df of MOTUs with < 80% match to BLAST (based on reviewer comment)
z80 <- z0 %>%
  filter(match < 0.80)
...
BLAST the fasta files created in the last step with blastn (BLAST+) in command line on the cluster. Then process all the text files created using that process here.
...
# write.csv(zs2, "data/pinebugs.samples.step2.negsub.swarm1.Z.csv")
...
# write.csv(cs2, "data/pinebugs.samples.step2.negsub.swarm1.C.csv")
...
# counting reads for supplemental table S1
```{r}
...
# Zeale step 3: subtract negatives, remove rest of non-arthropods, and match < 0.8
  # this is same as zs3 from above
step3.s <- zs3

# tally up read stats for samples
sum(apply(select(step3.s, starts_with("sample")), 2, sum)) # 4116475
mean(apply(select(step3.s, starts_with("sample")), 2, sum)) # 114346.5
sd(apply(select(step3.s, starts_with("sample")), 2, sum)) # 38731.1
length(apply(select(step3.s, starts_with("sample")), 2, sum)) # 36
38731.1 / sqrt(36) # 6455.183
...
length(apply(select(step3.n, starts_with("sample")), 2, sum)) # 36
...
# note: 44146 human reads; 9513 mouse reads

# Coleop step 3: subtract negatives, remove rest of non-arthropods, and match < 0.8
  # this is same as zs3 from above
step3.s <- cs3

# tally up read stats for samples
sum(apply(select(step3.s, starts_with("sample")), 2, sum)) # 2068121
mean(apply(select(step3.s, starts_with("sample")), 2, sum)) # 55895.16
sd(apply(select(step3.s, starts_with("sample")), 2, sum)) # 32712.56
length(apply(select(step3.s, starts_with("sample")), 2, sum)) # 37
32712.56 / sqrt(37) # 5377.912

step3.n <- cn3

# tally up read stats for samples
sum(apply(select(step3.n, starts_with("sample")), 2, sum)) # 3402
mean(apply(select(step3.n, starts_with("sample")), 2, sum)) # 200
sd(apply(select(step3.n, starts_with("sample")), 2, sum)) # 658.4878
length(apply(select(step3.n, starts_with("sample")), 2, sum)) # 17
658.4878/sqrt(17) # 159.7068
```
