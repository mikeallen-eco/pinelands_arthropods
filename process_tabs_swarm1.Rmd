...
zz_finID <- zz_fin  %>%
  ...
  select(id, finalID, bestmatch, phylum, class, order, family, genus, species,
         count, wt, starts_with("sample"), sequence, finaltax, blastnotes) 

# check to make sure all species level matches got a species in the final database
checkIDsZ = zz_finID %>% 
  select(id, bestmatch, finaltax, species, finalID, blastnotes) %>%
  filter(bestmatch > 0.99,
         is.na(species)) %>%
  arrange(desc(nchar(finaltax)))
  # note: this revealed ~ possible errors in which final_taxa_lookup didn't include species when it should have
    # that suggests that you can't rely on taxize get_taxa blindly without checking it carefully
    # for now, I manually updated the final_taxa_lookup table to correct these errors
zz_finID <- zz_finID %>% select(-finaltax, -blastnotes)
...
checkIDsC = cc_finID %>% 
  ...
  filter(bestmatch > 0.99,
         is.na(species)) %>%
  arrange(desc(nchar(finaltax)))
  # note: this revealed 5 errors in which final_taxa_lookup didn't include species when it should have
    # that suggests that you can't rely on taxize get_taxa blindly without checking it carefully
cc_finID <- cc_finID %>% select(-finaltax, -blastnotes)
...
```
# remove humans, mice, and farm animals
```{r}
...
# sum the reads across all samples
sum(select(zs1, starts_with("sample"))) # 2161052
...
colnames(zs2) # 7:42 are the samples
...
colnames(cs2) # 7:43 are the samples
...
unique(zs2$phylum)
zs2[is.na(zs2$phylum), c("id", "finalID")]
...
# check on phylum list and look at MOTUs with no phylum
unique(cs2$phylum)
cs2[is.na(cs2$phylum), c("id", "finalID")]
...
# sum the reads across all samples
sum(select(cs3, starts_with("sample"))) # 2069901
...
# what species; n = 186 (note: was 158 with swarm d = 7)
...
# what species; n = 105 (was 94 with swarm d = 5)
