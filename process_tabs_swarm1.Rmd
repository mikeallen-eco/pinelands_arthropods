...
full_taxa_lookup$finaltax[full_taxa_lookup$finaltax %notin% c(taxlistfull, taxlist80)]
...
# sum the reads across all samples
sum(select(cs3, starts_with("sample"))) # 2240250
...
# what genus; n = 227 (note: added 2 genera by examining MOTUs with match <80%)
...
# what families; n = 117 (note: added 2 families by examining MOTUs with match <80%))
...
# what orders; n = 16 (note: same as before)
...
# tally up read stats for samples
sum(apply(select(step3.s, starts_with("sample")), 2, sum)) # 2240250
mean(apply(select(step3.s, starts_with("sample")), 2, sum)) # 60547.3
sd(apply(select(step3.s, starts_with("sample")), 2, sum)) # 32799.31
length(apply(select(step3.s, starts_with("sample")), 2, sum)) # 37
32799.31 / sqrt(37) # 5392.173
