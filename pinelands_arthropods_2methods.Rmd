...
rm(coleop_all_step1, coleop_nonarth_lookup,
   nonarth, nonarth_step1)
...
rm(nonarth_zeale_step1, zeale_step1)
...
ggsave("figures/PlotE_species_accumulation_curve_spray_roll.jpg",
       dpi = 600, width = 6, height = 4)
```
# Spray vs Roller, conifer vs. deciduous NMDS
```{r}
library(vegan)
...
distances <- vegdist(mds_data_nb, method = "jaccard")
arthMDS <- isoMDS(distances, maxit = 300)

# plot results
arthMDS.df <- arthMDS$points %>%
  as.data.frame() %>%
  bind_cols(env.data)

arthMDS.df %>%
  ggplot() +
  geom_point(aes(x = V1, y = V2, color = method, shape = tree),
  size = 5) +
  coord_fixed(ratio = 1)
