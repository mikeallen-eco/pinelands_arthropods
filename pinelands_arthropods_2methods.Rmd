...
famlookup <- read_xlsx("data/arthropod.families_20220714.xlsx", 
...
lifemode <- read_xlsx("data/arthropod.families_20220728.xlsx", 
...
diet <- read_xlsx("data/arthropod.families_20220728.xlsx", 
...
# read in OTU data for coleop primer set and merge with family name
coleop_all <- read_xlsx("data/coleop.9.final.meta.xlsx", 
                    sheet = "Sheet1") %>%
  rename(final_ID = 'final species_name') %>%
  left_join(famlookup, by = "final_ID")

coleop <- coleop_all %>%
  filter(is.na(final_family)==F,
         final_family != "NA")
