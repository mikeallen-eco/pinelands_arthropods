...
ggsave("figures/PlotF_NMDS_by_method_and_tree_type.jpg",
...
# rarefy & diversity code from metabarcoding workshop
```{r}
# t_fishfarm is a sample x MOTU matrix filled with reads
min_depth_fishfarm <- min(rowSums(t_fishfarm))

# rarefaction curves
fishfarm.curve <-
  rarecurve(
    t_fishfarm_curve, # same as t_fishfarm
    step = 20,
    sample = min_depth_fishfarm,
    col = nrow(t_fishfarm), # number of samples
    label = T
  )

fishfarm_rare <- rrarefy(t_fishfarm, min_depth_fishfarm)

# Alpha diversity - Shannon index
# W/o rarefying
H_fish_norar <- diversity(t_fishfarm, index = "shannon", MARGIN = 1, base = exp(1))

# w/ rarefying
H_fish_rar <- diversity(fishfarm_rare, index = "shannon", MARGIN = 1, base = exp(1))



```

# Rarefied species accumulation curves by reads
```{r}

# MOTU accumulation curves by primer set (and order)
names(art_order) # to get column numbers of matrix
data_cols <- 8:44
coleop_rare_mat <- art_order %>%
...
coleop_data <- list(coleop = coleop_rare_mat,
                     zeale = zeale_rare_mat)

coleop_data2 <- list(coleop = apply(coleop_rare_mat,1,sum),
                     zeale = apply(zeale_rare_mat, 1, sum))
...
coleop_rare <- iNEXT(coleop_data[[1]][,-9], q=0, datatype="abundance",
                     knots = 100, endpoint = 3436)
zeale_rare <- iNEXT(coleop_data[[2]][,-9], q=0, datatype="abundance",
                    knots = 100, endpoint = 3436)

both_rare <- iNEXT(coleop_data2, q=0, datatype="abundance",
                   knots = 100, nboot = 50)
ggiNEXT(both_rare)

plot(zeale_rare$iNextEst$`sample:3CS`$m,
     zeale_rare$iNextEst$`sample:3CS`$qD,
     type = "l", col = "blue",
     ylim = c(0,140))

points(coleop_rare$iNextEst$`sample:3CS`$m,
     coleop_rare$iNextEst$`sample:3CS`$qD,
     type = "l", col = "red")

coleop_rare$DataInfo
$iNextEst
$AsyEst
...
rarefy_coleop <- rarefy(coleop_rare_mat[-9,], 3436, se = T)
rarefy_zeale <- rarefy(zeale_rare_mat[-9,], 3436, se = T)
...
ggplot(rarefy_df) +
  geom_boxplot(aes(x = primer, y = S)) +
  geom_point(aes(x = primer, y = S))


# now create a rarified version of the read matrixes for diversity metrics

coleop_rare_mat_rare <- rrarefy(coleop_rare_mat[-9,], 3436)
zeale_rare_mat_rare <- rrarefy(zeale_rare_mat[-9,], 3436)
...
ggplot(rarefy_df_wide) +
  geom_point(aes(x = zeale_S, y = coleop_S),
             size = 4, alpha = 0.6,
              color = "dodgerblue") +
  geom_smooth(aes(x = zeale_S, y = coleop_S), 
              method = "lm",
              color = "dodgerblue") +
    geom_abline(slope = 1, linetype = 2) +
  theme_bw() +
  theme(text = element_text(size = 14)) +
  labs(x = "MOTU richness (Zeale)",
       y = "MOTU richness (Coleop)")

ggplot(shannon_df,
       aes(x = zeale_shannon, y = coleop_shannon)) +
  ...
  theme_bw() +
  theme(text = element_text(size = 14)) +
  labs(x = "Shannon diversity (Zeale)",
       y = "Shannon diversity (Coleop)")

ggplot(simpson_df,
       aes(x = zeale_simpson, y = coleop_simpson)) +
  geom_point(size = 4, alpha = 0.6,
              color = "dodgerblue") +
  geom_smooth(method = "lm",
              color = "dodgerblue") +
  ...
  theme_bw() +
  theme(text = element_text(size = 14)) +
  labs(x = "MOTU evenness (Zeale)",
       y = "MOTU diversity (Coleop)")
...
rarefy_df_wide %>%
  ...
  mutate(name = case_when(name == "coleop_S" ~ "Coleop primer",
                          name == "zeale_S" ~ "Zeale primer")) %>%
  ...
  geom_violin(draw_quantiles = .5, aes(fill = name),
              alpha = 0.6) +
  geom_point(size = 4, alpha = .5, aes(color = name)) +  scale_fill_manual(values = c("firebrick", "steelblue")) +
  scale_color_manual(values = c("firebrick", "steelblue")) +
  theme_bw() +
  theme(text = element_text(size = 14)) +
  labs(x = "",
  ...
  annotate("text", x = 2.5, y = 160, 
           ...
           "\nColeop: ",
           round(exp(rarefy_stats$mean_lc),2), 
           ", 95% CI = [",
           round(exp(rarefy_stats$lcl_lc),2),
           ", ",
           round(exp(rarefy_stats$ucl_lc),2), 
           "]\n",
           "Zeale: ",
           round(exp(rarefy_stats$mean_lz),2),
           ", 95% CI = [",
           round(exp(rarefy_stats$lcl_lz),2),
           ", ",
           round(exp(rarefy_stats$ucl_lz),2),
           "]"),
           hjust = 1,
           color = "darkgray")
)
...
simpson_df %>%
  ...
  mutate(name = case_when(name == "coleop_simpson" ~ "Coleop primer",
                          name == "zeale_simpson" ~ "Zeale primer")) %>%
  ggplot(aes(x = name, y = value)) +
  geom_line(size = .5, alpha = .5, 
            aes(group = sample), 
            color = "darkgray") +
...
shannon_df %>%
  ...
  mutate(name = case_when(name == "coleop_shannon" ~ "Coleop primer",
                          name == "zeale_shannon" ~ "Zeale primer")) %>%
  ggplot(aes(x = name, y = value)) +
  geom_line(size = .5, alpha = .5, 
            aes(group = sample), 
            color = "darkgray") +
