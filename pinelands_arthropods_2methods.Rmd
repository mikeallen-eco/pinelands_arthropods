...
```
# multinomial models for feed data
...
fit.method.tree <- saveRDS("output/feed_multinomial_method.tree.rds")
fit.method <- saveRDS("output/feed_multinomial_method.rds")
fit.tree <- saveRDS("output/feed_multinomial_tree.rds")
...
```
# multinomial test in brms
https://discourse.mc-stan.org/t/example-with-family-multinomial/8707
https://discourse.mc-stan.org/t/interpret-parameters-from-multinomial-categorical-logit-regression/26374
```{r}
library(brms)
N <- 15
xcat = sample(c("hi", "bye"), N, replace = T)
dat <- data.frame(
    y1 = rbinom(N, 10, 0.3), 
    y2 = rbinom(N, 10, 0.5), 
    y3 = rbinom(N, 10, 0.7), 
...
fit <- brm(bf(y | trials(size)  ~ xcat, muy2 ~ x), data = dat, 
                family = multinomial(), prior = prior)
```

# multinomial test in jags
https://stackoverflow.com/questions/63444427/bayesian-multinomial-regression-using-rjags-package
```{r}
library(rjags)
#Data
set.seed(1)
