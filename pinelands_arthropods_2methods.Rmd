...
coleop_rare <- iNEXT(coleop_data[[1]], q=0, datatype="abundance")
zeale_rare <- iNEXT(coleop_data[[2]], q=0, datatype="abundance")
...
#### testing - saving if needed

coleop_rare_mat
coleop_squish <- apply(coleop_rare_mat, 2, sum) %>%
  as.matrix() %>%
  t()
accum.test <- specaccum(coleop_rare_mat,
                     y=data.frame(method = rep("coleop",
                                               nrow(coleop_rare_mat))),
                     factor='method',
  method='rarefaction', conditioned=FALSE, plotit=FALSE)
accum.test
```
