...
# make 'wide' db of read counts by taxa across both primers
# 
#   sds <- art_sp %>%
#   select(primer, sci, sample.sum) %>%
#   pivot_wider(names_from = primer, values_from = sample.sum)
#   filter(primer == "coleop")
#   
#   sdo <- art_order %>%
#   select(primer, order, sample.sum) %>%
#   pivot_wider(names_from = primer, values_from = sample.sum)
#   
#   sdf <- art_fam %>%
#   select(primer, family, sample.sum) %>%
#   pivot_wider(names_from = primer, values_from = sample.sum)
#   
#   sdg <- art_gen %>%
#   select(primer, genus, sample.sum) %>%
#   pivot_wider(names_from = primer, values_from = sample.sum)
...
# family presence/absence for arthropods (coleop primer)
...
accum.spray <- accumcomp(spray_mat_nb, 
...
mod1 <- fitspecaccum(both_mat, method = "exact", model = "lomolino"); AIC(mod1)
mod <- fitspecaccum(both_mat, method = "exact", model = "arrhenius"); AIC(mod)
mod <- fitspecaccum(both_mat, method = "exact", model = "gleason"); AIC(mod)
mod <- fitspecaccum(both_mat, method = "exact", model = "gitay"); AIC(mod)
mod <- fitspecaccum(both_mat, method = "exact", model = "asymp"); AIC(mod)
mod <- fitspecaccum(both_mat, method = "exact", model = "gompertz"); AIC(mod)
mod <- fitspecaccum(both_mat, method = "exact", model = "michaelis-menten"); AIC(mod)
mod <- fitspecaccum(both_mat, method = "exact", model = "logis"); AIC(mod)
mod1 <- fitspecaccum(both_mat, method = "exact", model = "weibull"); AIC(mod1)

coef(mod1)
fitted(mod1)
plot(both_mat)

plot(mod1, add = F, col=2, lwd=2)
pred = data.frame(samp = 1:80,
                  pred = predict(mod1, newdata = data.frame(Sites = 1:80)))
pred
```
