...
ggsave("figures/rev_Fig1_nosub_ordermotus_swarm1_1.png", height = 9, width = 7.5, dpi = 400)
...
ggsave("figures/rev_Fig1alt_ordermotussub_swarm1_2.png",
...
kable(ordereads_df, "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  cat(., file = "figures/table1_reads_tally_2.html")
...
ggsave("figures/rev_Fig2new_accumcurves.swarm1_16.png", dpi = 400, width = 11, height = 10)
...
# Intercept            3.67      0.11     3.45     3.90 1.00    10828     6523
# treeDeciduous        0.19      0.12    -0.04     0.43 1.00    10222     5893
# methodUnderstory     0.60      0.13     0.34     0.84 1.00    10486     5931***
# primerCOI           -0.71      0.12    -0.95    -0.47 1.00    10306     6053***


library(tidybayes)
quantile(spread_draws(Smod, b_treeDeciduous)$b_treeDeciduous, c(0.05, 0.95)) 
# -0.005369158  0.393345586
...
saveRDS(Z.df, "output/accum.motu.reads.Z.df.rds")
...
saveRDS(C.df, "output/accum.motu.reads.C.df.rds")
...
ggplot() +
  ...
  geom_line(aes(x = m, y = qD, color = Assemblage), data = filter(Z.df, Method == "Extrapolation"), lty = 3) +
  guides(color = "none") +
...
ggplot() +
  geom_line(aes(x = m, y = qD, color = Assemblage)) +
  geom_line(aes(x = m, y = qD, color = Assemblage), data = filter(C.df, Method == "Extrapolation"), lty = 3) +
  guides(color = "none") +
  scale_x_continuous(limits = c(0,50000))
```
