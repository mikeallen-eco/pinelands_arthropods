...
```
# Fig. 1 take 2 - accumulation curves using iNEXT
```{r}
...
# Fig. 2 - old version - construct accumulation curves using iNEXT
```{r}
...
# Intercept            3.68      0.11     3.46     3.91 1.00     5143     2581
# treeDeciduous        0.19      0.12    -0.05     0.43 1.00     5671     2868
# methodUnderstory     0.60      0.13     0.35     0.85 1.00     4614     2836***
# primerCOI           -0.72      0.12    -0.95    -0.48 1.00     5040     2869***
...
stxt <- paste0("COI vs. 16S \u03b2, 95% CI: [-0.95, -0.48]***",
           "\nUnderstory vs. Bark: [0.35, 0.85]***",
           "\nDeciduous vs. Conifer: [-0.05, 0.43]\n \n \n ")
...
rarefy_df20 %>%
  ...
  geom_violin(draw_quantiles = .5, aes(x = method, y = S, fill = tree),
              alpha = 0.6) +
    facet_wrap(~primer) +
  geom_point(size = 1.5, alpha = .75, aes(x = method, y = S, color = tree),
  ...
  geom_text(aes(x = 0.5, y = -65, label = txt),
           hjust = 0,
           data = S.anntext,
           size = annfont,
           color = "black")
...
#                  Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
# Intercept            0.50      0.07     0.36     0.63 1.00     4740     2712
# treeDeciduous        0.09      0.07    -0.05     0.23 1.00     5297     2474
# methodUnderstory     0.13      0.07    -0.01     0.27 1.00     5554     2983*
# primerCOI           -0.13      0.07    -0.27     0.02 1.00     5204     2718*
...
# Evenness boxplots
stxt <- paste0("COI vs. 16S \u03b2, 95% CI: [-0.27, 0.02]*",
           "\nUnderstory vs. Bark: [-0.01, 0.27]*",
...
simpson_df20 %>%
  ...
  geom_violin(draw_quantiles = .5, aes(x = method, y = simpson, fill = tree),
              alpha = 0.6) +
    facet_wrap(~primer) +
  geom_point(size = 1.5, alpha = .75, aes(x = method, y = simpson, color = tree),
             position = position_dodge(width = .9)) +
  # geom_point(size = 1.5, alpha = 1, aes(x = method, y = simpson, color = tree),
  #            position = position_dodge(width = .9)) +  
    scale_fill_manual(values = wes_palettes$Cavalcanti1[c(2,1)] ) +
  scale_color_manual(values = wes_palettes$Cavalcanti1[c(2,1)]) +
  theme_bw() +
...
#                  Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
# Intercept            1.17      0.21     0.76     1.58 1.00     5197     2875
# treeDeciduous        0.30      0.21    -0.12     0.71 1.00     5530     2778
# methodUnderstory     0.70      0.23     0.26     1.13 1.00     5515     2788***
# primerCOI           -0.35      0.22    -0.78     0.08 1.00     5127     2826*

library(tidybayes)
get_variables(Shanmod)
quantile(spread_draws(Shanmod, b_treeDeciduous)$b_treeDeciduous, c(0.05, 0.95)) 
# -0.03958887  0.63840591
...
# Shannon plot
stxt <- paste0("COI vs. 16S \u03b2, 95% CI: [-0.78, 0.08]*",
           "\nUnderstory vs. Bark: [0.26, 1.13]***",
           "\nDeciduous vs. Conifer: [-0.12, 0.71]\n \n \n ")
S.anntext <- data.frame(primer = c("16S", "COI"),
                        txt = c(stxt, ""))
...
shannon_df20 %>%
  ...
  geom_violin(draw_quantiles = .5, aes(x = method, y = shannon, fill = tree),
              alpha = 0.6) +
    facet_wrap(~primer) +
  geom_point(size = 1.5, alpha = .75, aes(x = method, y = shannon, color = tree),
             position = position_dodge(width = .9)) +  
    scale_fill_manual(values = wes_palettes$Cavalcanti1[c(2,1)] ) +
  scale_color_manual(values = wes_palettes$Cavalcanti1[c(2,1)]) +
...
ggsave("figures/rev_Fig2_rarefieddiv_20k_swarm1_4.png", width = 4.5, height = 7, dpi = 400)
...
ggsave(plot = gg, filename = "figures/rev_Fig3_rarediversity_swarm1_6.png", 
...
ggsave("figures/rev_Fig5_guilds_multinom_swarm1_1.jpg",
