...
# try again, this time including leaves vs. bark
# LEAVES V BARK
# Plot relative abundance of orders by method (for each primer set)
% of samples detected
```{r}
...
orders_df <- ordersCspray %>%
  ...
  mutate(taxorder = case_when(idlevel == "Species" ~ 1,
                              ...
                              idlevel == "Family" ~ 3,
                              idlevel == "Order" ~ 4),
         primorder = case_when(primer == "Leaves (COI)" ~ 1,
                              primer == "Tree bark (COI)" ~ 2,
                              primer == "Leaves (16S)" ~ 3,
                              primer == "Tree bark (16S)" ~ 4)) %>%
  mutate(engname = fct_reorder(.f = engname, .x = summotu, .desc = F),
         idlevel = fct_reorder(.f = idlevel, .x = taxorder, .desc = T),
...
ggsave("figures/rev6_ordermotussub3.png", height = 9, width = 7.5, dpi = 400)

```
# richness, diversity, evenness - by substrate, tree type, primer
```{r}
...
# get list of samples with low read depth (< 30000)
...
rarefy_coleop20 <-rarefy(select(coleop_rare_mat20,-sample.sum), minreads20,se = T)
rarefy_zeale20 <- rarefy(select(zeale_rare_mat20,-sample.sum), minreads20,se = T)
...
coleop_rare_mat_rare20 <- rrarefy(select(coleop_rare_mat20,-sample.sum), minreads20)

zeale_rare_mat_rare20 <- rrarefy(select(zeale_rare_mat20,-sample.sum), minreads20)
...
ggsave("figures/rev7_rarefieddiv5.png", width = 5, height = 7, dpi = 400)
```
